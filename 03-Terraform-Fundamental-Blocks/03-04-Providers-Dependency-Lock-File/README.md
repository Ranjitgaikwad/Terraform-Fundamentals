...
- Understand the importance of Dependency Lock File which is introduced in Terraform v1.14
...
- For AWS Provider, with version constraint `version = ">= 2.0.0"`, it is going to upgrade to latest `6.x.x` version with command `terraform init -upgrade` 
```t
